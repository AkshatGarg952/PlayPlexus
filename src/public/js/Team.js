...
let teams = []; // Changed from users to teams for clarity
let filteredTeams = []; // Store filtered teams for display
let currentPage = 1;
...
async function fetchTeams() {
    try {
        ...
        document.getElementById('usersContainer').innerHTML = '<p>Error loading teams. Please try again.</p>';
    }
}
...
function filterTeams(query) {
    if (!query) {
        filteredTeams = [...teams]; // Reset to all teams if query is empty
    } else {
...
function displayTeams(teamsToDisplay) {
    const teamsContainer = document.getElementById('usersContainer');
    ...
    teamsToDisplay.forEach(team => {
        const chatLink = userId
            ? `/chat1/${userId}/${team.id}`
            : `/chat1/${teamId}/${team.id}`;

        const teamCard = document.createElement('div');
        teamCard.className = 'user-card';
        teamCard.innerHTML = `
            ...
            <div class="card-content">
                ...
                <div class="card-actions">
                    <div class="action-row">
                        <button class="action-btn btn-profile" onclick="showProfile('${team.email}')">
                            <i class="fa-solid fa-user"></i> See Full Profile
                        </button>
                        <button class="action-btn btn-request" onclick="showRequestForm('${team.email}')">
...
function updateLoadMoreButton() {
    ...
    loadMoreBtn.style.display = filteredTeams.length > currentPage * teamsPerPage ? 'block' : 'none';
}
...
function showProfile(email) {
    ...
    profileDetail.innerHTML = `
        ...
        <div class="detail-item">
            ...
            <span>Sports: ${team.sports}</span>
        </div>
        <div class="detail-item">
            <i class="fa-solid fa-gamepad"></i>
            <span>Games: ${team.online_games}</span>
        </div>
    `;
...
function showRequestForm(email) {
    ...
    if (userObj && userObj._id) {
        baseActionUrl = `/api/requests/send2/${userObj._id}`;
    } else if (teamObj && teamObj._id) {
        baseActionUrl = `/api/requests/send2/${teamObj._id}`;
    } else {
    ...
    requestForm.action = `${baseActionUrl}/${team.id}`;
...
document.getElementById('requestForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Reset toggle to default state (Sport active, Game inactive)
            sportToggle.classList.add('active');
            gameToggle.classList.remove('active');
            document.getElementById('requestType').value = 'sport';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('loadMoreBtn').addEventListener('click', () => {
        ...
        displayTeams(filteredTeams.slice(0, end));
        updateLoadMoreButton();
    });
...
messageDiv1.appendChild(link);
        ...
        timeSpan.textContent = getCurrentTime();
        messageDiv.appendChild(timeSpan);
        messageDiv1.appendChild(timeSpan);
